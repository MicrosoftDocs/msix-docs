...
> [!TIP]
> Updating from your code is the reliable way to keep an app current. App Installer [OnLaunch update checks](app-installer/update-settings.md) run when the app is started through app activation, such as a menu item, a Start menu tile, an app execution alias, or a protocol handler. Other ways of starting the app, including some desktop shortcuts and taskbar buttons that launch the program directly, can start the installed version without checking for updates, so update from your code when you need a check to run on every launch.
...
If you are deploying your application using the App Installer file, any code driven updates you perform must make use of the [App Installer file APIs](./app-installer/app-installer-documentation.md#app-installer-file-apis). Doing so ensures that your regular App Installer file updates will continue to work. For intiating an App Installer based update from your code you can use [PackageManager.AddPackageByAppInstallerFileAsync](/uwp/api/windows.management.deployment.packagemanager.addpackagebyappinstallerfileasync) or [PackageManager.RequestAddPackageByAppInstallerFileAsync](/uwp/api/windows.management.deployment.packagemanager.requestaddpackagebyappinstallerfileasync). You can check if an update is available using the [Package.CheckUpdateAvailabilityAsync](/uwp/api/windows.applicationmodel.package.checkupdateavailabilityasync) API. Below is example code:
...
After you determined that an update is available, you can queue it up for download and install using the [AddPackageAsync](/uwp/api/windows.management.deployment.packagemanager.addpackageasync) API. It should also work to install an optional package as long as the main package is already installed on the device. The update will be applied the next time your app is shut down. After the app is restarted, the new version will be available to the user. Below is example code:

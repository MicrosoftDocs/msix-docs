...
>
> One exception to the previous paragraph is that even when the *ms-appinstaller* URI scheme is disabled, you can still enable installing from a web server by using an `.appinstaller` file that's at a local path. An `.appinstaller` file is an XML file containing URL references to a) a newer version of the `.appinstaller` file, and b) an MSIX package. And those urls can refer to remote `.appinstaller` and `.msix` files on a web server.
>
> Another thing to consider is authentication. The topic [App Installer Authentication Manager](/windows/msix/app-installer/app-installer-authentication-manager--overview) describes how to install from files that are gated by Azure AD authentication by using *ms-appinstaller*. If you're using an `.appinstaller` file that's at a local path, then that file can contain URLs that have `?msixauth=aad` at the end. That way you can still use use Azure AD authentication even without *ms-appinstaller*.

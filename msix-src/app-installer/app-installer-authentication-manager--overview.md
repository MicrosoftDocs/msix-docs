...
> 
> 2. The authentication is supported only for the MSIX itself. An `.appinstaller` file that the user double-clicks on can't itself be hosted on an AAD server; but it *can* reference (by URL) a newer `.appinstaller` file that *is* hosted on an AAD server. For more info, see [Installing Windows 10 apps from a web page](/windows/msix/app-installer/installing-windows10-apps-web).

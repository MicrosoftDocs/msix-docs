...
keywords: windows 10, deployment, msix, migrate, replace, installer, setup.exe, msi, clickonce
ms.assetid:  
---

# Replace an existing installer-based app with MSIX

When you move an existing desktop application to MSIX, you often have a version already installed on your users' machines through a classic installer, such as an **.exe** installer (often named **setup.exe**), a Windows Installer (**.msi**) package, or a **ClickOnce** deployment. This article explains how the MSIX version relates to that existing install, how to remove the previous version, and how to choose a deployment technology for the rollout.
...
* Both versions can be present on the machine at the same time, which can result in duplicate system integration and user-facing experiences, such as Start menu entries and file type associations.
...
| App Installer file | Distributing and auto-updating apps from your own website or from local media such as a file share, USB drive, or disc | [Install MSIX with App Installer](../app-installer/app-installer-root.md) |
...
| MSIX Core | Windows Server (with Desktop Experience) | [MSIX Core](../msix-core/msixcore.md) |

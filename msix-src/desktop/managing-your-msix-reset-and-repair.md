...
which expands to `C:\Users\<user>\AppData\Local\Packages\<PackageFamilyName>\`. This is the app's package-managed data. The most useful subfolders when diagnosing an issue are:
...
Full-trust packaged apps can also write to other user-writable locations outside the package data directory, as described in [Understand how packaged desktop apps run on Windows](/windows/msix/desktop/desktop-to-uwp-behind-the-scenes). If your app does this, capture those additional app-specific locations too, so your backup reflects the app's complete state.

...
+ __Your .NET application requires a version of the .NET Framework earlier than 4.6.2__. If you are packaging a .NET application, we recommend that your application target .NET Framework 4.6.2 or later. The ability to install and run desktop applications in an MSIX/APPX package was first introduced in Windows 10, version 1607 (also called the Anniversary Update), and this OS version includes the .NET Framework 4.6.2 by default. Later OS versions include later versions of the .NET Framework (for a full list of what versions of .NET are included in later versions of Windows 10, see [this article](https://docs.microsoft.com/dotnet/framework/migration-guide/versions-and-dependencies)).
  ...
  + 2.0 and 3.5: In our testing, applications that target these versions of the .NET Framework generally work from an MSIX/APPX package but may exhibit performance issues in some scenarios. In order for these applications to install and run from an MSIX/APPX package, the [.NET Framework 3.5 feature](https://docs.microsoft.com/dotnet/framework/install/dotnet-35-windows-10) must first be installed in the OS (this feature also includes .NET Framework 2.0 and 3.0). You should also test these applications thoroughly after you package them in an MSIX/APPX package.
